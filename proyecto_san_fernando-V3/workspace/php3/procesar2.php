<?php
    ...
    class Procesar extends Model{
    	...
    	public function build_report($year){
	    	...
	    	for($i=0; $i<12; $i++){
	    		$month = $i+1;
	    		$sql = $this->db->query("SELECT SUM(tiempo_Est_hrs) AS precio_total FROM prog_mantenimiento WHERE MONTH(fecha_ini_mas_tmp) = '$month' AND YEAR(fecha_ini_mas_tmp) = '$year'");	
	    		$precio_total[$i] = 0;
